...
/*EXPLAINATION:
	-main: To initiate program
		...
		- If the number of arguments is 2 (aka we are recivieng a string with all arguments) we use the function replicate_argv to simulate argv
		- Then we init the stack with the flag (free_arg in init stack function) to free simulated argv if argc = 2 (if needed else all remain the same)
		...
		- If the stack is not sorted:
			...
			- Else we call sort_stack function that trigers all the optimization moves to sort
		- Free the stack
*/
