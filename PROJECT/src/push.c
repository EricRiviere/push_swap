...
/*   Updated: 2024/10/04 10:54:42 by eriviere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* EXPLAINATION:
	...
	-4: Now that we're shure that we have a node to push, the pointer 
		(pushed_node that we created points to our src stack head
	-5: We move the head of src to src_first next node
	*IMPORTANT* src_first & dest_first is NOT the same as *src and *dest (both 
	start at the head of each stack and then are modified)
	-6: If we don't have src (meaning we just have 1 node in src stack),
	...
	-8: If dest stack is empty, its head will be pushed node and its next NULL
