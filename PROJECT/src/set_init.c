...
/*   Updated: 2024/10/04 09:56:13 by eriviere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*EXPLAINATION:
	-set_target_nodes: To set a target node from a in every node in b
		The target node will be the closest value in a that is greater than the
		value in b
		- Create a pointer to iterate through stack a (current_a)
		- Create a pointer to a target node that we'll update (almost) every
		iteration
		...
		- Reset current_a pointer to stack a head to iterate through all stack
		a in every node of stack b
		- 2nd while loop: Iterates through stack a and updates target node if
		fits conditions
		- while iterating through stack a, if we find a node in a which number
		is bigger than current b node number & smaller (closer) than
		temp_target_value, we update target node
		- Otherwise, we jump to the next node (current_a = current_a->next)
		- If we finish iterating through stack a and we didn't find a BIGGER && 
		CLOSER value (means b node number is the highest) its target node will 
		be the smallest in a)
		  *In this case target_node is not updated this is why its important to
		  init as NULL*
		- Else (means we found and updated target_node), b node target node 
		equals target_node
		- Jump to the next node on b to keep iterating (b = b->next)
*/

static void	set_target_nodes(t_node *a, t_node *b)
{
	t_node	*current_a;
	t_node	*target_node;
...
/*EXPLAINATION:
	-set_cost: To set the cost of pushing node b to a above its target node
		Te cost of pushing = relative position of b node (to median) + "" its 
		target node
		- Create variables to stack a & stack b length
		- Iterate through stack b to:
		-1: Set b push cost (initialize with its index && if its not avobe 
		median the cost will be stack b length - b index
		-2: Sum b target node cost (same way if above median cost = index,
		otherwise cost = target stack len (len_a) - b target node -from a- index)
...
/*EXPLAINATION:
	-set_cheapest: To set the node property cheapest to "true" if its push_cost 
	is the lowest among all nodes
